...
### 3.2 특수 블록 (4종, 배타적, 균등 무작위)
- **개수** = `8 × (스테이지 - 1)` (S1=0, S2=8, S3=16, S4=24, 보스=N/A)
- **① 갑옷** (연파랑 `#7fb3ff`): 공 데미지와 무관하게 항상 1만 받음, **HP 5 고정** (스테이지 무관)
- **② 파괴 불가** (진회색 `#3a3a3a`, "∞" 표시): 점수 없이 튕김만, 클리어 조건 제외
- **③ 공 복제** (금색 `#ffd700`): 파괴 시 해당 위치에서 ±60° 무작위 각도로 아래 방향 공 +1 발사
- **④ 강화** (보라 `#7c3aed`): HP × 2 (baseHp 기준)
- 4종 색상/설명은 캔버스 아래 `.legend` 패널에 표시됨
...
### 5.2 상수
- `MAX_STAGE = 5`
- `BRICK_TYPE`: `{NORMAL, ARMOR, INDESTRUCTIBLE, MULTIBALL, HARDENED, BOSS}`
...
- `createBall(x, y, dx, dy)`, `spawnBallAt(x, y)`
- `collideBricks(ball)` — 4면 겹침 깊이 기반 충돌 판정, 타입별 처리(갑옷=dmg 1, 파괴불가=튕김만, 공복제=spawn, 강화/보스=일반 데미지), 충돌 후 블록 밖으로 위치 보정. `b.w/b.h` 사용.
